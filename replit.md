...
- `scripts/migrate_gold_karat_columns.sql` - **Hotfix migration: must run manually in Supabase SQL Editor (idempotent).** Adds `gold_karat integer` to `orders`, `cad_requests`, and `manufacturing_orders` for older installs whose schema snapshot pre-dates the column. Without it, saving an order / CAD request / manufacturing order errors with `Could not find the 'gold_karat' column of 'orders' in the schema cache`.
