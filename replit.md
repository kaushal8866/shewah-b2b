...
- **Catalog price refresh on gold-rate change**: Saving a new gold rate on `app/gold-rates` automatically calls `recomputeCatalogPrices()` (in `lib/supabase.ts`), which recomputes `trade_price` and `mrp_suggested` for every active product using the same formula as the catalog edit page (gold + labour + diamonds + making + IGI, then ×1.28 / ×1.40). The page also exposes a manual "Recalculate catalog prices" button for use without changing the rate. The helper only writes when values differ and reports `{ updated, skipped, failed, error }` so partial failures surface in the alert.
