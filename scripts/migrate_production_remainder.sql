-- Production catch-up — REMAINDER
-- Only the pieces that did not apply on the previous run.
-- Idempotent. Paste into Supabase Dashboard → SQL Editor → New query → Run.
-- If you see a red error at the bottom of the editor, copy it and send it back.

create extension if not exists "uuid-ossp";

-- ── A. CAD/STL file arrays on manufacturing_orders ──────────────────────
ALTER TABLE manufacturing_orders ADD COLUMN IF NOT EXISTS cad_files      text[] DEFAULT '{}';
ALTER TABLE manufacturing_orders ADD COLUMN IF NOT EXISTS cad_file_names text[] DEFAULT '{}';

-- ── B. Lifecycle tracking on material_transactions ──────────────────────
...
-- ── C. Lifecycle column on manufacturing_orders ─────────────────────────
ALTER TABLE manufacturing_orders
  ADD COLUMN IF NOT EXISTS lifecycle text NOT NULL DEFAULT 'pending';

-- ── D. Negative-balance trigger respects 'pending' lifecycle ────────────
...
-- ── E. Karigar 48h share links table ────────────────────────────────────
...
-- ── F. Atomic float reservation helper ──────────────────────────────────
...
-- ── G. CAD partner public share links + responses + uploads ─────────────
...
BEGIN
  FOR cname IN
    ...
    WHERE conrelid = 'cad_revisions'::regclass AND contype = 'c'
...
-- ── I. CAD revision ACK + WhatsApp inbound token setting ────────────────
ALTER TABLE cad_revisions ADD COLUMN IF NOT EXISTS acknowledged_at timestamptz;
