...
export default function ShowcasePage() {
  ...
  // Map of product_id → interest data (local state only until submit)
  ...
  const load = useCallback(async () => {
    const res = await fetch(`/api/showcase/${encodeURIComponent(collectionId)}/${encodeURIComponent(partnerId)}`)
  ...
  function toggleInterest(product: Product) {
    setInterests(prev => {
  ...
  function updateNote(productId: string, field: 'note' | 'quantity_hint', value: string | number | null) {
    setInterests(prev => {
      const next = new Map(prev)
      const existing = next.get(productId) ?? { note: '' }
      if (field === 'note') {
        next.set(productId, { ...existing, note: (value ?? '') as string })
      } else {
        next.set(productId, { ...existing, quantity_hint: value === '' || value === null ? undefined : (value as number) })
      }
      return next
    })
  }
  ...
  async function handleSubmit() {
    ...
    const selections = Array.from(interests.entries()).map(([product_id, row]) => ({
      product_id,
      note: row.note || null,
      quantity_hint: row.quantity_hint ?? null,
    }))

    const res = await fetch(
      `/api/showcase/${encodeURIComponent(collectionId)}/${encodeURIComponent(partnerId)}`,
      {
        method: 'POST',
    ...
    return (
      <div className="min-h-screen bg-stone-950 flex items-center justify-center px-4">
        <div className="text-center max-w-sm w-full">
          ...
          <div className="flex items-center justify-center gap-2 mt-5 text-green-400 text-xs">
            <CheckCircle2 className="w-4 h-4" />
            Preferences saved to your account
          </div>
          <p className="text-stone-600 text-xs mt-3">You can revisit this link to update your selections at any time.</p>
        </div>
      </div>
    )
  }
              ...
              return (
                <div key={product.id}
                  ...
                  <div className="p-3">
                    ...
                    {/* Note fields (expanded when shortlisted + note button tapped) */}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {isShortlisted && (
                        <button
                          ...
                          className={`p-2.5 rounded-xl text-xs transition-colors active:scale-95 ${
                            showNote
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-stone-500 text-xs">Tap Submit when ready</p>
                </div>
              </div>
              <button
                ...
                Submit my shortlist ({shortlistedCount})
              </button>
            </div>
          )}
        </div>
