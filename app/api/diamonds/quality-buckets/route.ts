...
// Authenticated read so the catalog form + retailer comparison can list options.
export async function GET() {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .order('sort_order')
    .order('label')
  // Return empty list (instead of 500) when the table doesn't exist yet —
  // the migration is run manually by the operator and the rest of the app
  // should keep working in the meantime.
...
export async function POST(req: Request) {
  ...
  if (error) {
    ...
    if ((error as any).code === '23505') {
      return NextResponse.json({ error: `A quality bucket called "${label}" already exists.` }, { status: 409 })
    }
